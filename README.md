...
Download this repository, and navigate to the `dist` folder. You should see the following contents:

```
./dist/tag-4.1.0-swift-3.1-framework.zip
```

Download and unzip this file. Within the expanded folder contents, you should see the `LotameDMP.framework`. 
...
This section only applies to the **Carthage** and **Drag-and-Drop** installation methods. The `.framework` file generated by these two methods is something called a **Fat Binary**. This contains binaries for both Simulator and device -- both of which you'll need for app development. 
...
1. Under **Input Files**, add an entry for the following path: ```$(SRCROOT)/Carthage/Build/iOS/LotameDMP.framework```

Your screen should look something like this:

![run-script.png](./run-script.png)

**Note about file paths**: The **Input Files** location will vary, depending on your installation method. For example:

* **Carthage**: `$(SRCROOT)/Carthage/Build/iOS/LotameDMP.framework`
* **Drag-and-Drop**: `$(SRCROOT)/Carthage/LotameDMP.framework`
